...
const uint8_t DDRAM_Address[4][20] =
{
...
void LCD::WriteNibble(char byte, uint8_t nibbleBitPos)
{
	...
	/* Get high or low nibble according to 'nibbleBitPos' parameter */
	...
	{
		HAL_GPIO_WritePin(dataPins[i].GPIOx, dataPins[i].selectedPin, pinState[nibbleArray[i]]);
	}

	/* Pulse to EN pin to transfer nibble */
	HAL_GPIO_WritePin(en.GPIOx, en.selectedPin, GPIO_PIN_SET);
	HAL_Delay(1);
	HAL_GPIO_WritePin(en.GPIOx, en.selectedPin, GPIO_PIN_RESET);
	HAL_Delay(1);
}
...
void LCD::SetCursor(uint8_t row, uint8_t column)
{
	...
	LCD::WriteByte(GPIO_PIN_RESET, ( (1 << 7) | DDRAM_Address[row][column]));
		 ...
		 pinStruct_t& D7)
{
	...
	{
		LCD::WriteByte(GPIO_PIN_RESET, CommandSetting[i]);
	}
}
